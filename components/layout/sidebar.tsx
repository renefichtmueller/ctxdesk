...
export function Sidebar({ inboxCount = 0 }: SidebarProps) {
  ...
  const toggleExpanded = () => {
    ...
    setExpanded(next);
    try {
      localStorage.setItem("ctxdesk-sidebar-expanded", String(next));
    } catch {}
  };
  ...
  const navGroups: NavGroup[] = [
    ...
    },
    {
      groupLabel: "Workflow",
      items: [
        { href: "/inbox", icon: Inbox, label: "Inbox", badge: inboxCount > 0 ? inboxCount : undefined },
        { href: "/changelog", icon: ScrollText, label: "Changelog" },
        { href: "/queue", icon: Terminal, label: "Claude Queue" },
        { href: "/context", icon: Brain, label: "Claude Context" },
  ...
  return (
    <TooltipProvider delayDuration={200}>
      ...
      >
              ...
              <Link href="/" className="flex items-center gap-3 min-w-0">
                ...
                {isExpanded && (
                  <div className="min-w-0">
                    <span className="font-bold text-[#f1f5f9] text-[13px] tracking-tight block leading-none">
                      CtxDesk
                    </span>
                    <span className="text-[10px] text-[#64748b] leading-none mt-0.5 block">
                      Mission Control
                    </span>
                  </div>
                )}
          ...
          {navGroups.map((group, groupIdx) => (
            <div key={groupIdx} className="mb-1">
              ...
              {!isExpanded && groupIdx > 0 && (
                <div className="my-2 mx-4 h-px bg-[#1e293b]" />
              )}
                    ...
                    <Link
                      ...
                      className={cn(
                        ...
                        isActive
                          ...
                          : "text-[#64748b] hover:bg-[#1a2540] hover:text-[#cbd5e1]"
                      )}
                    >
                      {isActive && isExpanded && (
                        <div
                      ...
                      <div className="relative shrink-0">
                        ...
                        {item.badge !== undefined && item.badge > 0 && (
                          <span
                            className="absolute -top-1.5 -right-1.5 text-[8px] font-bold w-3.5 h-3.5 flex items-center justify-center rounded-full bg-red-500 text-white"
                          >
        ...
        <div
          className="flex flex-col pb-4 pt-3 gap-2 shrink-0"
          style={{ borderTop: "1px solid #1e293b" }}
        >
          {isExpanded && (
            <div className="flex items-center gap-2 px-4">
              <div className="w-1.5 h-1.5 rounded-full bg-emerald-400 live-pulse" />
              <span className="text-[11px] text-[#64748b]">Mac Studio · :3002</span>
            </div>
          )}
